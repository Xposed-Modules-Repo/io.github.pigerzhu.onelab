...
import static io.github.pigerzhu.onelab.contract.SettingsKeys.KEY_ENABLE_ZHUANZHUAN_SPLIT_RULES;
...
public final class ZhuanzhuanSplitRulesScreen {
    private final MainActivity host;
    private final Ui ui;
    private final SettingsStore settings;

    public ZhuanzhuanSplitRulesScreen(
            MainActivity host,
            Ui ui,
    ...
    public View card() {
        ...
        toggle.setChecked("1".equals(
                settings.getGlobal(KEY_ENABLE_ZHUANZHUAN_SPLIT_RULES, "0")));
        toggle.setOnCheckedChangeListener((button, enabled) ->
                settings.setGlobal(
                        KEY_ENABLE_ZHUANZHUAN_SPLIT_RULES,
                        enabled ? "1" : "0"));
        row.addView(toggle);
        return card;
    }
